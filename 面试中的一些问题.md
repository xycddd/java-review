...
```
用ClassLoader加载类，是不会导致类的初始化（也就是说不会执行clinit方法）.Class.forName(...)加载类，不但会将类加载，还会执行会执行类的初始化方法
...
```
- *** 在java 编程中，我们都需要和***打交道，通常一个应用当中会用到很多***，我们会调用一个控件的诸如addXXXListener()等方法来增加***，但往往在释放对象的时候却没有记住去删除这些***，从而增加了内存泄漏的机会。
...
https://blog.csdn.net/loveqishan/article/details/89393122
