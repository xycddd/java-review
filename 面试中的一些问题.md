...
（1）方法A中调用方法B，方法A中调用的方法B没有增强的效果，（2）调用的是对象本身而不是代理对象，所以没有效果（很多其它注解失效的原因也是如此）（3）好像可以采用编译期织入的方式
...
200. rabbitMQ常见问题（rabbitMQ如何保证没有重复分发）？
https://my.oschina.net/u/4275752/blog/4272268
